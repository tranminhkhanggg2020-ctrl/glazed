...
public class SusChunkFinder extends Module {
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        ...
        // ── Bắt Gói tin Âm Thanh (Sửa lỗi 1.21.4 getId) ──
        else if (event.packet instanceof PlaySoundS2CPacket packet) {
            RegistryEntry<SoundEvent> soundEntry = packet.getSound();
            // Lấy ID an toàn thông qua RegistryKey để tương thích với cấu trúc mới
            ...
            if (threat > 0) {
                // Tích hợp điểm âm thanh vào bộ đếm
    ...
    private void evaluateBlockThreat(BlockPos pos, BlockState state) {
        ...
        if (state.isAir()) {
            ...
            // Ghost Sonar nâng cao chống GrimAC (Sửa lỗi constructor 1.21.4)
    ...
    // Hàm Sonar tàng hình mô phỏng chuyển động
            ...
            for (int y = -2; y <= 2 && !found; y++) {
                for (int z = -2; z <= 2 && !found; z++) {
                    ...
                    if (hit.getType() == HitResult.Type.BLOCK && hit.getBlockPos().equals(visibleAnchor)) {
                        ...
                        // Đã Bổ sung mc.player.horizontalCollision cho bản 1.21.4
